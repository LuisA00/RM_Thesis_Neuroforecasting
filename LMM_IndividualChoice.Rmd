...
Individual view percent ~ ISC neural activity

```{r, echo = FALSE}
WScore_ISC <- lmer(W_score ~ (Type) + scale(Pos_arousal) + scale(Neg_arousal) + (NAcc_ISC) + (AIns_ISC) + (MPFC_ISC) + NAcc_ISC : Type + AIns_ISC : Type + MPFC_ISC : Type +  (1|Participant) + (1|Trailer), data=AllSubs_NeuralActivation)
summary(WScore_ISC)
r.squaredGLMM(WScore_ISC)
