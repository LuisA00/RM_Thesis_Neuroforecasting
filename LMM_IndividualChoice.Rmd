...
AllSubs_NeuralActivation_pre_flip <- read.csv('/Users/luisalvarez/Documents/GitHub/RM_Thesis_Neuroforecasting/ProcessedData/AllSubs_NeuralActivation_pre_flip.csv')
```

# M1: Behavioral data
```{r, echo = FALSE}
M1 <- lmer(scale(W_score)~ (Type) + scale(Pos_arousal) + scale(Neg_arousal) + 
             scale(Pos_arousal):Type + scale(Neg_arousal):Type +
...
Neg_arousal <- AllSubs_NeuralActivation %>% pull(Neg_arousal)
Pos_arousal <- AllSubs_NeuralActivation %>% pull(Pos_arousal)

M3_data <- data.frame(W_score, Pos_arousal, Neg_arousal, NAcc_whole, AIns_whole, MPFC_whole) 
...
M5 <- lmer(W_score~ (Type) + scale(Pos_arousal) + scale(Neg_arousal) + 
             ...
             + scale(NAcc_ISC):Type + scale(AIns_ISC):Type + scale(MPFC_ISC):Type +
             (1|Participant) + (1|Trailer), data = AllSubs_NeuralActivation_pre_flip)
summary(M5)
r.squaredGLMM(M5)
...
r.squaredGLMM(M7)
```
# Compare neural activation middle and ISC models 
```{r}
AIC(M1, M5, M7)
```
