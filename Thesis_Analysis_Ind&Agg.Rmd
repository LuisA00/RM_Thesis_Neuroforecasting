...
ggpairs(Individual_BothTrailers_df[, c("Type", "W_score", "Pos_arousal_scaled", "Neg_arousal_scaled", "A_score", "F_score", "AIns_DP1", "AIns_DP2")], columns = 2:ncol(Individual_BothTrailers_df[, c("Type", "W_score", "Pos_arousal_scaled", "Neg_arousal_scaled", "A_score", "F_score", "AIns_DP1", "AIns_DP2")]), 
        aes(color = Type)) 

#ggplot(Individual_BothTrailers_Horror_df, aes(x = (AIns_DP2), y = (W_score), color=H_preference)) +
#  geom_point() +
...
# IndBoth: Peak models
```{r}
# What if, I try mean(peak: peak+2) ???
...
```{r}
ggpairs(Individual_BothTrailers_df[, c("Type", "W_score", "NAcc_ind_peaks_pos", "AIns_ind_peaks_pos", "MPFC_ind_peaks_pos")], columns = 2:ncol(Individual_BothTrailers_df[, c("Type", "W_score", "AIns_ind_peaks_pos", "MPFC_ind_peaks_pos")]), aes(color = Type)) 
...
IndBoth_GLMo_Neuro <- lmer(scale(W_score) ~
                        Type +   
                        scale(NAcc_GLM_short) +
                        scale(AIns_GLM_short) +
                        scale(vmPFC_GLM_short) +
                        Type:scale(NAcc_GLM_short) + 
                        Type:scale(AIns_GLM_short) + 
                        Type:scale(vmPFC_GLM_short) +
                        (1|Participant) + (1|Trailer), 
                        data = Individual_BothTrailers_df) 
...
IndBoth_GLMo_models <- modelsummary(IndBoth_GLMo_list, stars=TRUE, 
                           #statistic = c("t = {statistic}", "s.e. = {std.error}, p-val = {p.value}"),
                           #estimate  = "{estimate} ({std.error}) {stars}",
                           estimate = c("ß={estimate}, t = {statistic}, s.e. = {std.error}, p-val = {p.value}"),
                           statistic = NULL,
                           #output = "html",
                           output = "dataframe",
...
IndBoth_Affect_and_ISC <- (scale(W_score) ~
                        Type +              
                        scale(Pos_arousal_scaled) + 
                        scale(Neg_arousal_scaled) + 
                        scale(NAcc_ISC) +
...
# Making Figures
# Individual Choice: NAcc Invididual peak and choice rating.
```{r}
IndBoth_PNeuro
plot_model(IndBoth_PNeuro, type = "pred", terms = "NAcc_ind_peaks_pos", show.values = TRUE)
...
```{r}
Individual_BothTrailers_rm_nans_df = select(Individual_BothTrailers_df, "Type", "AIns_DP2", "NAcc_ind_peaks_pos", "W_score") %>%
  na.omit(Individual_BothTrailers_df)

ggplot(Individual_BothTrailers_rm_nans_df, aes(x = (NAcc_ind_peaks_pos), y = (W_score))) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_cor() 
  #geom_text(aes(label = Trailer), vjust = 2)
...
ggplot(Individual_BothTrailers_rm_nans_df, aes(x = (NAcc_ind_peaks_pos), y = (W_score),  color=Type)) + #
