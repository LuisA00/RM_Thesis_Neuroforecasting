...
for (sub_id in participant_list)
{
    ...
    # Create empty vectors for modal peak.
    NAcc_DP <- c()
    AIns_DP <- c()
    Amyg_DP <- c()
    MPFC_DP <- c()
    ...
    # Create empty vectors for individual peak.
    NAcc_IP <- c()
    AIns_IP <- c()
    Amyg_IP <- c()
    MPFC_IP <- c()
    ...
    for (trailer_id in trailer_list)  
    {
      ...
      current_NAcc_IP_id <- which.max(NAcc_timeseries)
      current_AIns_IP_id <- which.max(AIns_timeseries)
      current_Amyg_IP_id <- which.max(Amyg_timeseries)
      current_MPFC_IP_id <- which.max(MPFC_timeseries)
      
      Current_NAcc_IP <- get_MP(NAcc_timeseries, current_NAcc_IP_id, 0)
      Current_AIns_IP <- get_MP(AIns_timeseries, current_AIns_IP_id, 0)
      Current_Amyg_IP <- get_MP(Amyg_timeseries, current_Amyg_IP_id, 0)
      Current_MPFC_IP <- get_MP(MPFC_timeseries, current_MPFC_IP_id, 0)
      
      Current_NAcc_IP1 <- get_MP(NAcc_timeseries, current_NAcc_IP_id, 1)
      Current_AIns_IP1 <- get_MP(AIns_timeseries, current_AIns_IP_id, 1)
      Current_Amyg_IP1 <- get_MP(Amyg_timeseries, current_Amyg_IP_id, 1)
      Current_MPFC_IP1 <- get_MP(MPFC_timeseries, current_MPFC_IP_id, 1)
      
      Current_NAcc_IP2 <- get_MP(NAcc_timeseries, current_NAcc_IP_id, 2)
      Current_AIns_IP2 <- get_MP(AIns_timeseries, current_AIns_IP_id, 2)
      Current_Amyg_IP2 <- get_MP(Amyg_timeseries, current_Amyg_IP_id, 2)
      Current_MPFC_IP2 <- get_MP(MPFC_timeseries, current_MPFC_IP_id, 2)
      
      Current_NAcc_IP3 <- get_MP(NAcc_timeseries, current_NAcc_IP_id, 3)
      Current_AIns_IP3 <- get_MP(AIns_timeseries, current_AIns_IP_id, 3)
      Current_Amyg_IP3 <- get_MP(Amyg_timeseries, current_Amyg_IP_id, 3)
      Current_MPFC_IP3 <- get_MP(MPFC_timeseries, current_MPFC_IP_id, 3)
      
      # Append modal peaks to vector.
      ...
      # Append individual peaks to vector.
      NAcc_IP[[length(NAcc_IP)+1]] = Current_NAcc_IP[1]
      AIns_IP[[length(AIns_IP)+1]] = Current_AIns_IP[1]
      Amyg_IP[[length(Amyg_IP)+1]] = Current_Amyg_IP[1]
      MPFC_IP[[length(MPFC_IP)+1]] = Current_MPFC_IP[1]

      NAcc_IP1[[length(NAcc_IP1)+1]] = Current_NAcc_IP1[1]
      AIns_IP1[[length(AIns_IP1)+1]] = Current_AIns_IP1[1]
      Amyg_IP1[[length(Amyg_IP1)+1]] = Current_Amyg_IP1[1]
      MPFC_IP1[[length(MPFC_IP1)+1]] = Current_MPFC_IP1[1]
      
      NAcc_IP2[[length(NAcc_IP2)+1]] = Current_NAcc_IP2[1]
      AIns_IP2[[length(AIns_IP2)+1]] = Current_AIns_IP2[1]
      Amyg_IP2[[length(Amyg_IP2)+1]] = Current_Amyg_IP2[1]
      MPFC_IP2[[length(MPFC_IP2)+1]] = Current_MPFC_IP2[1]
      
      NAcc_IP3[[length(NAcc_IP3)+1]] = Current_NAcc_IP3[1]
      AIns_IP3[[length(AIns_IP3)+1]] = Current_AIns_IP3[1]
      Amyg_IP3[[length(Amyg_IP3)+1]] = Current_Amyg_IP3[1]
      MPFC_IP3[[length(MPFC_IP3)+1]] = Current_MPFC_IP3[1]
    ...
    current_sub_df$NAcc_IP <- NAcc_IP
    current_sub_df$AIns_IP <- AIns_IP
    current_sub_df$Amyg_IP <- Amyg_IP
    current_sub_df$MPFC_IP <- MPFC_IP
    
    current_sub_df$NAcc_IP1 <- NAcc_IP1
    current_sub_df$AIns_IP1 <- AIns_IP1
    current_sub_df$Amyg_IP1 <- Amyg_IP1
    current_sub_df$MPFC_IP1 <- MPFC_IP1
    
    current_sub_df$NAcc_IP2 <- NAcc_IP2
    current_sub_df$AIns_IP2 <- AIns_IP2
    current_sub_df$Amyg_IP2 <- Amyg_IP2
    current_sub_df$MPFC_IP2 <- MPFC_IP2
    
    current_sub_df$NAcc_IP3 <- NAcc_IP3
    current_sub_df$AIns_IP3 <- AIns_IP3
    current_sub_df$Amyg_IP3 <- Amyg_IP3
    current_sub_df$MPFC_IP3 <- MPFC_IP3
...
all_participants_final_df$NAcc_IP <- as.numeric(as.character(all_participants_final_df$NAcc_IP))
all_participants_final_df$NAcc_IP1 <- as.numeric(as.character(all_participants_final_df$NAcc_IP1))
all_participants_final_df$NAcc_IP2 <- as.numeric(as.character(all_participants_final_df$NAcc_IP2))
all_participants_final_df$NAcc_IP3 <- as.numeric(as.character(all_participants_final_df$NAcc_IP3))

all_participants_final_df$AIns_IP <- as.numeric(as.character(all_participants_final_df$AIns_IP))
all_participants_final_df$AIns_IP1 <- as.numeric(as.character(all_participants_final_df$AIns_IP1))
all_participants_final_df$AIns_IP2 <- as.numeric(as.character(all_participants_final_df$AIns_IP2))
all_participants_final_df$AIns_IP3 <- as.numeric(as.character(all_participants_final_df$AIns_IP3))

all_participants_final_df$Amyg_IP <- as.numeric(as.character(all_participants_final_df$Amyg_IP))
all_participants_final_df$Amyg_IP1 <- as.numeric(as.character(all_participants_final_df$Amyg_IP1))
all_participants_final_df$Amyg_IP2 <- as.numeric(as.character(all_participants_final_df$Amyg_IP2))
all_participants_final_df$Amyg_IP3 <- as.numeric(as.character(all_participants_final_df$Amyg_IP3))

all_participants_final_df$MPFC_IP <- as.numeric(as.character(all_participants_final_df$MPFC_IP))
all_participants_final_df$MPFC_IP1 <- as.numeric(as.character(all_participants_final_df$MPFC_IP1))
all_participants_final_df$MPFC_IP2 <- as.numeric(as.character(all_participants_final_df$MPFC_IP2))
all_participants_final_df$MPFC_IP3 <- as.numeric(as.character(all_participants_final_df$MPFC_IP3))
...
IndBoth_PNeuro <- lmer(scale(W_score) ~
                        ...
                        data = all_participants_final_df) 

# Create affect and neuro sequence model.
IndBoth_Affect_and_PNeuro <- lmer(scale(W_score) ~
                        Type +              
                        scale(PA_score_centered) + 
                        scale(NA_score_centered) + 
                        scale(NAcc_IP) +
                        #scale(AIns_IP) +
                        #scale(Amyg_IP) + 
                        #scale(MPFC_IP) +
                        Type:scale(PA_score_centered) +
                        Type:scale(NA_score_centered) +
                        Type:scale(NAcc_IP) + 
                        #Type:scale(AIns_IP) +
                        #Type:scale(Amyg_IP) +
                        #Type:scale(MPFC_IP) +
...
IndBoth_P_models <- modelsummary(IndBoth_P_list, stars=TRUE, 
                           ...
                           estimate = c("ß={estimate}; t={statistic}; s.e.={std.error}; p-val={p.value}{stars}"),
                           statistic = NULL,
                           #output = "html",
                           output = "dataframe",
...
```{r}
ggplot(all_participants_final_df, aes(x = Amyg_IP, y = W_score, color=Type)) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_cor() 
```
...
ggpairs(all_participants_final_df[, c("Type", "W_score", "PA_score_centered", "NA_score_centered", "NAcc_IP", "AIns_IP", "Amyg_IP", "MPFC_IP")], columns = 2:ncol(all_participants_final_df[, c("Type", "W_score",  "PA_score_centered", "NA_score_centered","NAcc_IP", "AIns_IP", "Amyg_IP", "MPFC_IP")]), 
        aes(color = Type))
...
```{r}
ggplot(all_aggregate_combined, aes(x = AIns_DP, y = log(Gross_M1_updated), )) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_cor() +
  geom_text(aes(label = Trailer), vjust = 2)
...
```{r}
ggplot(all_aggregate_combined, aes(x = Amyg_DP2, y = log(Gross_M1_updated), color=Type.y)) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_cor() +
  geom_text(aes(label = Trailer), vjust = 2)
...
```{r}
ggpairs(all_aggregate_combined[, c("Type.y", "Gross_M1_updated_log", "NAcc_DP2", "AIns_DP2", "MPFC_DP2", "Amyg_DP2")], columns = 2:ncol(all_aggregate_combined[, c("Type.y", "Gross_M1_updated_log", "NAcc_DP2", "AIns_DP2", "MPFC_DP2", "Amyg_DP2")]), 
        aes(color = Type.y)) 
```
