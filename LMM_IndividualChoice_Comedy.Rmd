...
```{r}
ggplot(AllSubs_IndividualChoice_C, aes(x = NAcc_onset, y = (W_score_scaled))) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_cor() 
  # + geom_text(aes(label = Trailer), vjust = 2)
...
Avg_peaks_yes_list <- modelsummary(Avg_peaks_yes_list, stars=TRUE, statistic=NULL, output = "dataframe", gof_omit = "F|RMSE|Log|Num.Obs.|SD (Observations)") %>% select(-part, -statistic)
Avg_peaks_yes_list
```
```{r}
